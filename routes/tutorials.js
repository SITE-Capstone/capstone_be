...
// Gets the completed statuses of one tutorial in completed_tutorials per user_id and tutorial_id
router.get("/single", security.requireAuthenticatedUser, async (req, res, next) => {
  try {
    //Defining the parameters
    ...
    //Model Functions
    const completedTutorials = await Tutorial.fetchSingleTutorial(id, tutorial_id);
    const publicCompletedTutorial = await Tutorial.makePublicCompletedTutorials(completedTutorials);

    return res.status(200).json({ completedTutorials: publicCompletedTutorial });
  } catch (err) {
    next(err);
  }
});
